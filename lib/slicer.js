...
PendingRead.prototype.trigger = function trigger(state, force) {
  ...
  // error.
  if (length === 0) {
    this.callback(undefined, 0, buffer || new Buffer(0), offset);
    return true;
  }
  ...
  if ((bufferedLength === 0) && (state.error !== consts.NO_ERROR)) {
    this.callback(state.error, 0, buffer || new Buffer(0), offset);
    return true;
  }
  ...
  this.callback(undefined, endOffset - origOffset, buffer, origOffset);
  return true;
}
 ...
 */
Slicer.prototype.read = function readInto(arg1, arg2) {
  var length;
  var callback;
  ...
  this.readInto(undefined, 0, length, callback);
}
